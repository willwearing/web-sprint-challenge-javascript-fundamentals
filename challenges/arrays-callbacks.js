...
const lowerCaseAnimalNames = zooAnimals.map((lowerCase) =>
  lowerCase.animal_name.toLowerCase()
);
...
const lowPopulationAnimals = zooAnimals.filter(
  (animalPopulation) => animalPopulation.population < 5
);
...
zooAnimals.reduce(
  (accumulator, reduce) => (populationTotal = accumulator + reduce.population),
  0
);
